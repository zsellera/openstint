...
1. **remote end**, which has to be terminated by a *termination resistor*, matching the line's characteristic impedance
2. **"business end"**, which should connect to an impedance transformer (balun)

The line's characteristic impedance is about 400 Ω, but depends on:
* distance from the ground (closer means lower impedance)
* wire separation (closer means lower impedance)
* wire thickness (higher gauge means lower impedance)
...
**PROTIP:**
* To reduce common mode interference (from other legit sources like maritime radio), lay down the wires in parallel, with 90° bends.
...
The way SDRs operate, allow the downconversion of out-of-band signals to the baseband. In practical terms, the radio might see interfering radio at 15 MHz or FM transmission at 90 MHz, despite it is tuned to 5 MHz (the transponder frequency).

To prevent this from happening, a band-pass filter is highly recommended, while an FM-rejection filter is the 2nd best thing to have.
...
On a laptop, install [SDRAngel](https://www.sdrangel.org/), an open-source radio software, and open the radio with it. Tune it to 5 MHz, 5 M Sample Rate (SR), apply the 1.75M low-pass filter. Depending on the balun/preamp, check the "Bias T" box.
