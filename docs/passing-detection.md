...
* Lay down the pickup loop in a section where the lowest speed is 1 m/s. Shitty transponder placements result in double-peaks. To prevent double-detections, the car must pass the loop in ca. 250 ms (hence the speed limit).
...
In this setup, the fieldlines cross the loop as the transponder closes and leaves the loop, and create a zero when the transponder is right in the middle. Maximum field strength is about when the transponder is somewhere before the loop. Mind the "somewhere": this setup is not usable to detect passing duration.
